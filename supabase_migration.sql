-- ============================================================
-- Swasthya Sathi — Supabase Database Schema
...
-- ============================================================
-- 5. SEED DATA — DEPARTMENTS, SYMPTOMS, DISEASES, HOSPITALS, DOCTORS
-- ============================================================
...
insert into departments (name, nepali_name) values
  ...
  ('Orthopedics', 'हड्डी तथा जोर्नी रोग विभाग'),
  ('Ophthalmology', 'आँखा रोग विभाग'),
  ('Dermatology', 'छाला रोग विभाग'),
  ('Neurology', 'स्नायु रोग विभाग'),
  ('Tropical & Infectious Disease', 'सरुवा तथा ट्रोपिकल रोग विभाग'),
  ('Pulmonology & Respiratory Medicine', 'छाती तथा श्वासप्रश्वास रोग विभाग'),
  ('Gastroenterology & Hepatology', 'ग्यास्ट्रो तथा कलेजो रोग विभाग'),
  ('High-Altitude & Emergency Medicine', 'हिमाली तथा आकस्मिक चिकित्सा विभाग');
...
insert into symptoms (name, nepali_name) values
  ...
  ('Runny nose', 'नाक बग्ने'),
  ('Sneezing', 'हाच्छिउँ आउने'),
  ...
  ('Blurred vision', 'आँखा धमिलो हुने'),
  ('Skin rash', 'छालामा दाग'),
  ('Itching', 'चिलाउने'),
  ('Ear pain', 'कान दुख्ने'),
  ('Weight loss', 'तौल घट्ने'),
  ('Night sweats', 'राति पसिना आउने'),
  ('Swelling', 'सुन्निने'),
  ('Nosebleed', 'नाकबाट रगत आउने'),
  ('Chest tightness', 'छाती जाम भए जस्तो');
...
insert into diseases (department_id, name, nepali_name, description, nepal_context) values
  -- ENT
  ((select id from departments where name = 'ENT'), 'Chronic Sinusitis', 'पुरानो साइनसाइटिस', 'Inflammation of sinuses lasting 12+ weeks', 'Worsened by dust and pollution.'),
  ((select id from departments where name = 'ENT'), 'Allergic Rhinitis', 'एलर्जी राइनाइटिस', 'Allergic inflammation of nasal airways', 'Common due to pollen and dust.'),
  ((select id from departments where name = 'ENT'), 'Acute Otitis Media', 'कानको संक्रमण', 'Middle ear infection', 'Common in children after colds.'),
  ((select id from departments where name = 'ENT'), 'Tonsillitis', 'टन्सिलको सुन्निने', 'Inflammation of tonsils', 'Common in winter months.'),
  -- General Medicine
  ((select id from departments where name = 'General Medicine'), 'Typhoid Fever', 'टाइफाइड', 'Systemic bacterial infection', 'Waterborne illness, monsoon peak.'),
  ((select id from departments where name = 'General Medicine'), 'Type 2 Diabetes', 'मधुमेह', 'Chronic metabolic disorder', 'Rising in urban areas.'),
  ((select id from departments where name = 'General Medicine'), 'Common Cold', 'सामान्य रुघा', 'Viral upper respiratory infection', 'Very common in winter and rainy season.'),
  ((select id from departments where name = 'General Medicine'), 'Viral Fever', 'भाइरल ज्वरो', 'Fever caused by viral infection', 'Common due to infections and monsoon.'),
  ((select id from departments where name = 'General Medicine'), 'Dengue Fever', 'डेङ्गु ज्वरो', 'Mosquito-borne viral infection', 'Endemic across plains and urban valleys during post-monsoon.'),
  -- Cardiology
  ((select id from departments where name = 'Cardiology'), 'Coronary Artery Disease', 'हृदय धमनी रोग', 'Narrowing of coronary arteries', 'Linked to hypertension.'),
  ((select id from departments where name = 'Cardiology'), 'Hypertension', 'उच्च रक्तचाप', 'High blood pressure', 'Common in urban population.'),
  ((select id from departments where name = 'Cardiology'), 'Angina', 'एन्जाइना', 'Chest pain from reduced blood flow to heart', 'Triggered by exertion.'),
  ((select id from departments where name = 'Cardiology'), 'Heart Failure', 'हृदयघात', 'Heart unable to pump sufficiently', 'Related to untreated hypertension.'),
  -- Orthopedics
  ((select id from departments where name = 'Orthopedics'), 'Osteoarthritis', 'गठिया', 'Degenerative joint disease', 'Common from physical labor.'),
  ((select id from departments where name = 'Orthopedics'), 'Back Pain', 'ढाड दुख्ने', 'Pain in the back', 'Often due to physical labor.'),
  -- Ophthalmology
  ((select id from departments where name = 'Ophthalmology'), 'Cataract', 'मोतियाबिन्दु', 'Clouding of eye lens', 'Age and UV related.'),
  -- Dermatology
  ((select id from departments where name = 'Dermatology'), 'Fungal Infection', 'फंगल संक्रमण', 'Fungal skin infection', 'Humid climate trigger.'),
  -- Neurology
  ((select id from departments where name = 'Neurology'), 'Migraine', 'माइग्रेन', 'Severe throbbing headache', 'Stress and altitude triggers.'),
  ((select id from departments where name = 'Neurology'), 'Stroke', 'मस्तिष्काघात', 'Brain blood supply interruption', 'Hypertension related.'),
  -- Tropical
  ((select id from departments where name = 'Tropical & Infectious Disease'), 'Scrub Typhus', 'स्क्रब टाइफस', 'Bacterial disease from mites', 'Prevalent during agricultural season.'),
  ((select id from departments where name = 'Tropical & Infectious Disease'), 'Acute Gastroenteritis', 'झाडापखाला', 'Stomach flu / infection', 'Frequent outbreaks during monsoon.'),
  -- Pulmonology
  ((select id from departments where name = 'Pulmonology & Respiratory Medicine'), 'COPD', 'दम रोग', 'Chronic obstructive pulmonary disease', 'High prevalence due to biomass fuel smoke.'),
  ((select id from departments where name = 'Pulmonology & Respiratory Medicine'), 'Pulmonary Tuberculosis', 'क्षयरोग', 'Airborne bacterial lung infection', 'High priority public health issue.'),
  -- Gastroenterology
  ((select id from departments where name = 'Gastroenterology & Hepatology'), 'Hepatitis E', 'हेपाटाइटिस इ', 'Liver inflammation from HEV', 'Fecal-oral transmission, extreme risk to pregnant women.'),
  -- High-Altitude
  ((select id from departments where name = 'High-Altitude & Emergency Medicine'), 'HAPE / HACE', 'लेक लाग्ने', 'High altitude pulmonary/cerebral edema', 'Life-threatening above 2,500m.');
...
-- ============================================================
-- 6. HELPER FUNCTION — symptom-based disease lookup
-- ============================================================
...
-- ============================================================
-- 7. HELPER FUNCTION — health score calculation
-- ============================================================
